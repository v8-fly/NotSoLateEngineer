...
public class SwiggyX {
    ...
    public synchronized boolean orderItem(String userId) {
        if (inventory > 0) { inventory--; return true; }
        return false;
    }
...
dbConnectionPool.release();    // never reached
...
try {
    ...
    dbConnectionPool.release(); // always runs
}
```
...
Thread 2: inventoryLock → paymentLock  ← different = circular wait possible

// NO DEADLOCK — same order
Thread 1: paymentLock → inventoryLock
Thread 2: paymentLock → inventoryLock  ← same = Thread 2 waits at first lock
```
...
saveToDatabase(order)
    .thenApplyAsync(r -> chargePayment(r))
    .thenApplyAsync(r -> notifyRestaurant(r))
